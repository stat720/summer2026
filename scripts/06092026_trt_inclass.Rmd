...
```{r}
model1 <- lm(Serum_haptoglobin_mg.dL ~ 1 + Trt * Day, data = df_pigs)

model1 <- lm(Serum_haptoglobin_mg.dL ~ 1 + Trt + Day + Trt:Day,
             data = df_pigs)
summary(model1)
str(df_pigs)
```
...
```
```{r}
sqrt(sigma(model1)^2 / 16)
```

```{r}
emmeans(model1, ~ Trt:Day, at = list(Trt = "A"), 
        contr = list(c(-1, 1)))

```

```{r}
sqrt(2*sigma(model1)^2 / 16)
```
...
```{r}
emmeans(model1, ~ Trt:Day, at = list(Day = "38"), 
        contr = list(c(1, -1, rep(0, 4))))
```
